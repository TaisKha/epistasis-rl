...
class EpistasisEnv(gym.Env):
    ...
    def normalize_reward(self, current_reward):
        ...
        if normalized_reward > 0.5 or normalized_reward < - 0.5:
            print("normalized reward > 0.5 or < - 0.5. normalized reward = ", normalized_reward, "curr_reward = ", current_reward)
            normalized_reward = current_reward - 0.5
...
if __name__ == "__main__":
    ...
    env = EpistasisEnv()
    if WANDB:
        wandb.init(project="epistasis", entity="taisikus", name="negative-rewards", config={
